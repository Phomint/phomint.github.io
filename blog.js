...
async function loadPost(post) {
    ...
    waitForElement('#quantum-viz').then(() => initQuantumAnimation()).catch(() => { });
}
...
function initQuantumAnimation() {
    ...
    const qubitSphere = container.querySelector('.qubit-sphere');
    const resultEl = container.querySelector('.measurement-result');
    const btn = container.querySelector('.measurement-btn');
    const state0 = container.querySelector('.state-0');
    const state1 = container.querySelector('.state-1');
    const coinFace = container.querySelector('.coin-face');

    if (!qubitSphere || !resultEl || !btn || !state0 || !state1) return;

    let isSuperposition = true;
    qubitSphere.classList.add('superposition-active');

    const currentLang = localStorage.getItem('selectedLanguage') || 'pt';
    const texts = {
        pt: {
            measuring: 'Medindo...',
            result0: 'Resultado: Estado |0>',
            result1: 'Resultado: Estado |1>',
            reset: 'Tentar Novamente',
            measure: 'Medir Qubit'
        },
        en: {
            measuring: 'Measuring...',
            result0: 'Result: State |0>',
            result1: 'Result: State |1>',
            reset: 'Try Again',
    ...
    btn.onclick = () => {
        if (!isSuperposition) {
            ...
            qubitSphere.classList.add('superposition-active');
            coinFace.classList.remove('collapsed');
            state0.classList.remove('hidden');
            state1.classList.remove('hidden');
            resultEl.classList.remove('show');
            btn.textContent = texts.measure;
            return;
        }

        // Measure
        btn.disabled = true;
        btn.textContent = texts.measuring;
        
        setTimeout(() => {
            isSuperposition = false;
            qubitSphere.classList.remove('superposition-active');
            
            // Random result (0 or 1) - Math.random() is uniformly distributed [0, 1)
            const result = Math.floor(Math.random() * 2); // 0 or 1
            
            coinFace.classList.add('collapsed');
            if (result === 0) {
                state1.classList.add('hidden');
                resultEl.textContent = texts.result0;
            } else {
                state0.classList.add('hidden');
                resultEl.textContent = texts.result1;
            }
            
            resultEl.classList.add('show');
            btn.disabled = false;
            btn.textContent = texts.reset;
        }, 1200);
    };
}
